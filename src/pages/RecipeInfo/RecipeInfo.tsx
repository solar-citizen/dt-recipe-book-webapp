...
const RecipeInfo = () => {
  ...
  const handleFilterClick = async (type: 'ingredient' | 'country', value: string) => {
    await navigate(`/recipes?${type}=${encodeURIComponent(value)}`)
  }
        ...
        <section className='flex-1'>
          <div className='flex flex-col md:flex-row gap-6 mb-8'>
            ...
            <div className='flex-1'>
              ...
              <button
                onClick={async () => {
                  await handleFilterClick('country', recipe.strArea)
                }}
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 gap-4'>
              {recipe.ingredients.map(({ ingredient, measure }, index: number) => (
                <button
                  key={index}
                  onClick={async () => {
                    await handleFilterClick('ingredient', ingredient)
                  }}
